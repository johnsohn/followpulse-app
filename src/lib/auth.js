...
import { useUser, updateUser,reloadUser } from "./db";
...
import saEvent from "./simpleanalytics"
...
function useAuthProvider() {
  ...
  const signinWithProvider = (name,redirect) => {
    return (
      supabase.auth
        .signInWithOAuth({
          provider: name,
          options: {
            redirectTo: redirect || `${window.location.href}`,
          },
        })
        .then(handleError)
  ...
  const signinWithMagicLink = (email,redirect) => {
    return supabase.auth
      .signInWithOtp({
        email,
        options: {
          emailRedirectTo: redirect || `${window.location.href}`,
        },
      })
      .then(handleError);
  };
...
function useFormatUser(user) {
  ...
  return useMemo(() => {
    ...
    var tier = "free";
    if (user.nMessagesSub == 5000)
      tier = "gold";
    else if (user.nMessagesSub == 20000)
      tier = "elite";
...
function useIdentifyUser(user, { enabled }) {
  ...
  const [didFire,setFire] = useState('');
  useEffect(() => {
