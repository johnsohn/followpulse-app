...
console.log('got this from stripe',CheckoutProvider)
...
export default function Home() {
  ...
  const [payStageLoading,setPayStageLoading] = useState(true);
  const [isBackableFromPayment,setBackableFromPayment] = useState(false);
  const auth = useAuth();

  useEffect(()=>{
    if (stage == 'pay'){
      setTimeout(()=>{setPayStageLoading(false)},1000)
      setTimeout(()=>{setBackableFromPayment(true)},2000)
    }
    else
    {
      setPayStageLoading(true);
      setBackableFromPayment(false);
    }
  },[stage])
...
*/
  ...
  useEffect(() => {
    if (selectedPlan && !!auth.user) {
      fetch(`/api/checkout_sessionsv2?plan=${encodeURIComponent(selectedPlan)}${auth.user.email ? "&email="+encodeURIComponent(auth.user.email) : ""}`, {
        method: "POST",
      })
        .then((res) => res.json())
  ...
  // Then use the stored clientSecret when needed
  ...
  const fetchClientSecret = useCallback(() => {
    ...
    return fetch(`/api/checkout_sessionsv2?plan=${encodeURIComponent(selectedPlan)}${auth.user.email ? "&email="+encodeURIComponent(auth.user.email) : ""}`, {
      method: "POST",
    })
      .then((res) => res.json())
  ...
  */
  ...
  return (
    <div className="w-full min-h-screen bg-white md:bg-zinc-900 flex flex-col md:flex-row">
      ...
      {stage == "pay" && (
      <div className="flex flex-col max-w-sm mx-auto p-0 text-center md:w-1/2 md:max-w-none md:bg-white md:justify-center md:p-4 md:relative">

      {!payStageLoading && isBackableFromPayment && (
      ...
      <p className="text-xs text-center mt-4 text-gray-500">
        Without cancellation, I accept that Chatbot AI App will automatically charge me {plans.find(e=>e.id==selectedPlan).price} USD every {plans.find(e=>e.id==selectedPlan).name} until I cancel. Cancel anytime.
